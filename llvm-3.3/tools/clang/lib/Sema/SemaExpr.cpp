...
//wanzhifan in 
QualType Sema::CheckElementWiseOperands(ExprResult &LHS, ExprResult &RHS, SourceLocation Loc){
  ...
  const ConstantArrayType *lArray = dyn_cast<ConstantArrayType>(lhsType);
  const ConstantArrayType *rArray = dyn_cast<ConstantArrayType>(rhsType);
  const llvm::APInt lNum = lArray -> getSize();
  const llvm::APInt rNum = rArray -> getSize();
  QualType lElementType = lArray->getElementType().getUnqualifiedType();
  QualType rElementType = rArray->getElementType().getUnqualifiedType();

  if(lElementType == rElementType && lNum == rNum){
    if(!lhsExpr->isRValue()){
      Qualifiers tmp;
      ImplicitCastExpr *LToR = ImplicitCastExpr::Create(Context,
        Context.getUnqualifiedArrayType(lhsExpr->getType().getUnqualifiedType(), tmp),
        CK_LValueToRValue, const_cast<Expr *>(lhsExpr), 0, VK_RValue);
      LHS = LToR;
    }
    ...
    if(!rhsExpr->isRValue()){
      Qualifiers tmp;
      ImplicitCastExpr *LToR = ImplicitCastExpr::Create(Context,
        Context.getUnqualifiedArrayType(rhsExpr->getType().getUnqualifiedType(), tmp),
        CK_LValueToRValue, const_cast<Expr *>(rhsExpr), 0, VK_RValue);
      RHS = LToR;
    }
    return LHS.get()->getType();
  }else{
    this->Diag(Loc, diag::err_constant_array_not_the_same) <<
...
//wanzhifan in 
  if(ConstantArrayType::classof(LHS.get()->getType().getTypePtr())&&
     ...
     ElementWisePragmaOn){
    return CheckElementWiseOperands(LHS, RHS, Loc);
  }
//wanzhifan out
...
//wanzhifan in 
  if(ConstantArrayType::classof(LHSExpr->getType().getTypePtr()) &&
     ConstantArrayType::classof(RHS.get()->getType().getTypePtr()) &&
     ElementWisePragmaOn){
    const Expr *rhsExpr = RHS.get();
    const Type *lhsType = LHSExpr->getType().getTypePtr();
    const Type *rhsType = rhsExpr->getType().getTypePtr();
    const ConstantArrayType *lArray = dyn_cast<ConstantArrayType>(lhsType);
    const ConstantArrayType *rArray = dyn_cast<ConstantArrayType>(rhsType);
    const llvm::APInt lNum = lArray->getSize();
    const llvm::APInt rNum = rArray->getSize();
    ...
    if(lElementType == rElementType && lNum == rNum){
      if(!LHSExpr->isLValue()){
        this->Diag(Loc, diag::err_typecheck_array_not_modifiable_lvalue)<<LHSExpr->getType();
        return QualType();
      }
      Expr::isModifiableLvalueResult IsLV = LHSExpr->isModifiableLvable(Context, &Loc);
      if(IsLV == Expr::MLV_ConstQualified){
        this->Diag(Loc, diag::err_typecheck_assign_const);
        return QualType();
      }
      if(!rhsExpr->isRValue()){
        Qualifiers tmp;
        ImplictCastExpr *LToR = ImplicitCastExpr::Create(Context,
          Context.getUnqualifiedArrayType(rhsExpr->getType().getUnqualifiedType(), tmp),
          CK_LValueToRValue, const_cast<Expr *>(rhsExpr), 0, VK_RValue);
        RHS = LToR;
      }
